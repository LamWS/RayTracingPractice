...
void D3D12RaytracingProceduralGeometry::InitializeScene()
{
    ...
    // Setup materials.
    {
        ...
        // Emission
        XMFLOAT4 dark = XMFLOAT4(0, 0, 0, 0);
        XMFLOAT4 light = XMFLOAT4(1,1,1,1);
    	
        UINT offset = 0;
        // Analytic primitives.
        {
            using namespace AnalyticPrimitive;
            SetAttributes(offset + AABB, red, light);
            SetAttributes(offset + Spheres, ChromiumReflectance,dark, 1);
            offset += AnalyticPrimitive::Count;
        }
        ...
        {
            using namespace VolumetricPrimitive;
            SetAttributes(offset + Metaballs, ChromiumReflectance,dark, 1);
            offset += VolumetricPrimitive::Count;
        }
        ...
        {
            using namespace SignedDistancePrimitive;
            SetAttributes(offset + MiniSpheres, green,dark);
            SetAttributes(offset + IntersectedRoundCube, green,dark);
            SetAttributes(offset + SquareTorus, ChromiumReflectance,dark, 1);
            SetAttributes(offset + TwistedTorus, yellow, dark,0, 1.0f, 0.7f, 50, 0.5f );
            SetAttributes(offset + Cog, yellow, dark, 0, 1.0f, 0.1f, 2);
            SetAttributes(offset + Cylinder, red, dark);
            SetAttributes(offset + FractalPyramid, green, dark, 0, 1, 0.1f, 4, 0.8f);
        }
    }
...
void D3D12RaytracingProceduralGeometry::BuildProceduralGeometryAABBs()
{
    ...
    // Set up AABBs on a grid.
    {
        ...
        {
            using namespace AnalyticPrimitive;
            m_aabbs[offset + AABB] = InitializeAABB(XMINT3(1.5, 0, 1.5), XMFLOAT3(2, 3, 2));
